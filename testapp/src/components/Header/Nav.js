        ...
        <nav className='flex flex-col gap-4 justify-start text-base'>

            {
                menuItems.map((item, index) => (
                    item.link ?
                        <Link className='flex gap-3 items-center text-left rounded-lg px-2 py-2 font-medium hover:bg-slate-100/10 group' href={item.link} key={index}>
            ...
            <Link href={"/dynamic-username"} className='flex gap-3  items-center text-left rounded-lg px-2 py-2 font-medium hover:bg-slate-100/10 group group'>
